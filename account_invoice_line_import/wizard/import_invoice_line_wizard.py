...
class InvoiceLineImport(models.TransientModel):
    ...
    def _input_fields(self):
        ...
        res = {
            ...
            'cantidad': {'method': self._handle_cantidad},
    ...
    def _handle_descripcion(self, field, line, invoice, ail_vals):
        if not ail_vals.get('name'):
            ail_vals['name'] = line['descripcion'] or '/'

    def _handle_cantidad(self, field, line, invoice, ail_vals):
        if not ail_vals.get('cantidad'):
            qty = line[field]
            ail_vals['cantidad'] = str2float(qty, self.decimal_separator) or 1
    ...
    def _handle_product(self, field, line, invoice, ail_vals):
        ...
        if not products:
            products = prod_mod.search(
                [('name', '=', input)])
        if not products:
            msg = _("Producto '%s' not found !") % input
            self._log_line_error(line, msg)
            return
        elif len(products) > 1:
        ...
        if line.get('cantidad') and 'cantidad' not in ail_vals:
            self._field_methods['cantidad']['method'](
                'cantidad', line, invoice, ail_vals)
        qty = ail_vals.get('cantidad') or 1
