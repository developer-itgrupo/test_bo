...
class AccountStandardExcel(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, wizard):
        ...
        def _header_sheet(sheet):
            ...
            sheet.write(3, 6, _('Print on %s') % report.print_time)

            sheet.write(3, 4, _('Start Date : %s ') % wizard.date_from if wizard.date_from else '')
            sheet.write(3, 5, _('End Date : %s ') % wizard.date_to if wizard.date_to else '')

            sheet.write(2, 4, _('Target Moves:'), bold)
            sheet.write(2, 5, _('All Entries') if wizard.target_move == 'all' else _('All Posted Entries'))
...
AccountStandardExcel('report.dvit_account_multi_report.report_account_standard_excel', 'account.report.standard.ledger')
