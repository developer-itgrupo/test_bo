...
class AccountStandardLedgerLines(models.TransientModel):
    ...
    type = fields.Selection([('0_init', 'Initial'), ('1_init_line', 'Init Line'), ('2_line', 'Line'), ('3_compact',
                                                                                                       'Centralized'), ('4_total', 'Total'), ('5_super_total', 'Super Total')], string='Type')
...
class AccountStandardLedger(models.TransientModel):
    ...
    partner_select_ids = fields.Many2many(comodel_name='res.partner', string='Partners', help='If empty, get all partners')
    ...
    month_selec = fields.Selection([(1, '01 Junary'), (2, '02 Febuary'), (3, '03 March'), (4, '04 April'), (5, '05 May'), (6, '06 June'),
                                   ...
                                   string='Month')
    result_selection = fields.Selection([('recevable', 'Recevable'),
                                         ('payable', 'Payable'),
                                         ('ar_ap', 'AR/AP')
                                         ], string="Partner's", required=True, default='ar_ap')
    ...
    def on_change_type_ledger(self):
        ...
        if self.result_selection == 'payable':
            return {'domain': {'account_in_ex_clude': [('internal_type', '=', 'payable')]}}
        if self.result_selection == 'receivable':
            return {'domain': {'account_in_ex_clude': [('internal_type', '=', 'receivable')]}}
        return {'domain': {'account_in_ex_clude': [('internal_type', 'in', ('payable', 'receivable'))]}}
    ...
    def action_view_lines(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'tree,form',
            'views': [(self.env.ref('dvit_account_multi_report.view_aged_tree').id if self.type_ledger == 'aged' else False, 'tree'), (False, 'form')],
    ...
    def print_pdf_report(self):
        self.ensure_one()
        self._compute_data()
        return self.env['report'].get_action(self, 'dvit_account_multi_report.report_dvit_account_multi_report')

    def print_excel_report(self):
        self.ensure_one()
        self._compute_data()
        return self.env['report'].get_action(self, 'dvit_account_multi_report.report_account_standard_excel')
    ...
    def _sql_init_balance(self):
        ...
        FROM
       	...
       	WHERE
            ...
            AND ((%s AND acc.centralized = TRUE) OR acc.internal_type NOT IN ('payable', 'receivable') OR (aml.full_reconcile_id IS NOT NULL AND mif.id IS NULL))
        GROUP BY
            group_by_key
        HAVING
            CASE
    ...
    def _sql_lines(self):
        ...
        date_range AS
            (
                SELECT
                    %s AS date_current,
                    DATE %s AS date_less_30_days,
                    DATE %s - INTEGER '30' AS date_less_60_days,
                    DATE %s - INTEGER '60' AS date_less_90_days,
                    DATE %s - INTEGER '90' AS date_less_120_days,
                    DATE %s - INTEGER '120' AS date_older
            )
        ...
        WHERE
            ...
            AND (CASE
                    ...
                    ELSE acc.internal_type IN ('payable', 'receivable') AND (aml.full_reconcile_id IS NULL OR mif.id IS NOT NULL)
                END)
            AND aml.date <= %s
            AND aml.journal_id IN %s
            AND aml.account_id IN %s
            AND (%s IN ('account','journal','analytic') OR aml.partner_id IN %s)
            AND (%s != 'analytic' OR aml.analytic_account_id IN %s)
            AND NOT (%s AND acc.centralized = TRUE)
        ...
        params = [
            ...
            # date_range
            self.report_id.date_from,self.report_id.date_from,self.report_id.date_from,self.report_id.date_from,self.report_id.date_from,self.report_id.date_from,
    ...
    def _sql_lines_centralized(self):
        query = """
        INSERT INTO account_report_standard_ledger_line
        ...
        WHERE
            ...
            AND aml.journal_id IN %s
            AND aml.account_id IN %s
            AND (%s AND acc.centralized = TRUE)
        GROUP BY
            aml.account_id
        """
    ...
    def _search_account(self):
        ...
        if type_ledger in ('partner', 'aged',):
            ...
            if result_selection == 'payable':
                acc_type = ('payable',)
            elif result_selection == 'receivable':
                acc_type = ('receivable',)
            else:
