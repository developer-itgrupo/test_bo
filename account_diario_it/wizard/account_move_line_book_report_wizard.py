...
class account_move_line_book_report_wizard(osv.TransientModel):
	...
	@api.model
	def get_wizard(self):
		return self.env['automatic.fiscalyear'].get_wizard('Libro Diario',self.id,'account.move.line.book.report.wizard','account_diario_it.view_account_move_line_book_report_wizard_form','default_period_ini','default_period_end')
	...
	def do_rebuild(self):
		...
		self.env.cr.execute("""select count(*)  from account_move_line_book_report""")
		...
		if self.type_show == 'csv':
			...
			#CSV
			sql_query = """	COPY (SELECT * FROM account_move_line_book_report )TO '"""+direccion+docname+"""'   WITH DELIMITER ',' CSV HEADER			
							"""
		...
		if self.type_show == 'excel':
			...
			tam_col = [11.2,10,9,9,11,11,7,10,11,13,45,9,14.2,14,14,25,16,10,10,10]
