...
class AccountMassReconcile(models.Model):
    ...
    def _get_total_unrec(self):
        ...
        for rec in self:
            if rec.date_start and rec.date_stop and rec.partner_id.id:
                rec.unreconciled_count = obj_move_line.search_count(
                    [('account_id', '=', rec.account.id),
                    ('reconciled', '=', False),('partner_id', '=', rec.partner_id.id),('move_id.fecha_contable', '>=', rec.date_start),('move_id.fecha_contable', '<=', rec.date_stop)])
            elif rec.date_start and rec.date_stop and rec.partner_id.id == False:
                rec.unreconciled_count = obj_move_line.search_count(
                    [('account_id', '=', rec.account.id),
                    ('reconciled', '=', False),('move_id.fecha_contable', '>=', rec.date_start),('move_id.fecha_contable', '<=', rec.date_stop)])
            elif (not rec.date_start) and (not rec.date_stop) and rec.partner_id.id:
                rec.unreconciled_count = obj_move_line.search_count(
                    [('account_id', '=', rec.account.id),
                    ('reconciled', '=', False),('partner_id', '=', rec.partner_id.id)])
            else:
                rec.unreconciled_count = obj_move_line.search_count(
                    [('account_id', '=', rec.account.id),
                    ('reconciled', '=', False)])
    ...
    def _prepare_run_transient(self, rec_method):
        ...
        'partner_id': rec_method.task_id.partner_id.id,
        'date_start': rec_method.task_id.date_start,
        'date_stop': rec_method.task_id.date_stop,
    ...
    def open_unreconcile(self):
        ...
        if self.date_start and self.date_stop and self.partner_id.id:
            lines = obj_move_line.search(
                [('account_id', '=', self.account.id),
                ('reconciled', '=', False),('partner_id', '=', self.partner_id.id),('move_id.fecha_contable', '>=', self.date_start),('move_id.fecha_contable', '<=', self.date_stop)])
        elif self.date_start and self.date_stop and self.partner_id.id == False:
            lines = obj_move_line.search(
                [('account_id', '=', self.account.id),
                ('reconciled', '=', False),('move_id.fecha_contable', '>=', self.date_start),('move_id.fecha_contable', '<=', self.date_stop)])
        elif (not self.date_start) and (not self.date_stop) and self.partner_id.id:
            lines = obj_move_line.search(
                [('account_id', '=', self.account.id),
                ('reconciled', '=', False),('partner_id', '=', self.partner_id.id)])
        else:
            lines = obj_move_line.search(
                [('account_id', '=', self.account.id),
                ('reconciled', '=', False)])
