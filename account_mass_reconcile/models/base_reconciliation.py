...
class MassReconcileBase(models.AbstractModel):
    ...
    def _from(self, *args, **kwargs):
        return ("FROM account_move_line inner join account_move on account_move.id = account_move_line.move_id ")
    ...
    def _where(self, *args, **kwargs):
        ...
        if self.date_start and self.date_stop:
            ...
            where = ("""WHERE account_move_line.account_id = %s 
                     AND account_move.fecha_contable >= %s and account_move.fecha_contable <= %s
                     AND NOT account_move_line.reconciled""")
            # it would be great to use dict for params
            # but as we use _where_calc in _get_filter
            # which returns a list, we have to
            # accomodate with that
            params = [self.account_id.id, self.date_start, self.date_stop]
        else:
            where = ("""WHERE account_move_line.account_id = %s 
                     AND NOT account_move_line.reconciled""")
            # it would be great to use dict for params
