...
class db_backup(models.Model):
    ...
    path_install_postgres = fields.Char('Direccion de intalacion de Postgres(si es windows)')
    ...
    def schedule_maintenance(self):
        ...
        path_postgres = db.path_install_postgres if db.path_install_postgres else ''
        shell = True if db.path_install_postgres else False
        comand1 = path_postgres + 'reindexdb --host=localhost --port=5432 --username='+odoo.tools.config['db_user']+' --dbname=postgres'
        comand2 = path_postgres + 'reindexdb --host=localhost --port=5432 --username='+odoo.tools.config['db_user']+' --dbname='+db.name
        comand3 = path_postgres + 'reindexdb --host=localhost --port=5432 --username='+odoo.tools.config['db_user']+' --system --dbname='+db.name
        comand4 = path_postgres + 'vacuumdb --host=localhost --port=5432 --username='+odoo.tools.config['db_user']+' --dbname='+db.name+' --full --analyze --verbose'
        if not db.path_install_postgres:
            comand1 = shlex.split(comand1)
            comand2 = shlex.split(comand2)
            comand3 = shlex.split(comand3)
            comand4 = shlex.split(comand4)
        os.putenv('PGPASSWORD', odoo.tools.config['db_password'])
        subprocess1 = Popen(comand1,shell=shell)
        subprocess2 = Popen(comand2,shell=shell)
        subprocess3 = Popen(comand3,shell=shell)
        subprocess4 = Popen(comand4,shell=shell)

        vals = {'backup': True, 'obs_backup': 'Fue un exito','vacuum':True,'obs_vacuum':'Fue un exito','reindex':True,'obs_reindex':'Fue un exito','date':datetime.date.today()}
        log = self.env['log.maintenace.db']
        log_exist = log.search([('date','=',datetime.date.today())],limit=1)
        if log_exist:
            log_exist.write(vals)
        else:
            log.create(vals)
    ...
    def schedule_backup(self):
        vals = {'backup': True, 'obs_backup': 'Fue un exito', 'date': datetime.date.today()}
        log = self.env['log.maintenace.db']
        log_exist = log.search([('date', '=', datetime.date.today())], limit=1)
        if log_exist:
            log_exist.write(vals)
