...
class AccountMoveLineImport(models.TransientModel):
	...
	def _input_fields(self):
		...
		"""
		res = {
			'account_id': {'method': self._handle_account,'required':True},
			'debit': {'method': self._handle_debit, 'required': True},
			'credit': {'method': self._handle_credit, 'required': True},
			'partner_id': {'method': self._handle_partner},
			'product_id': {'method': self._handle_product},
			'date_maturity': {'method': self._handle_date_maturity},
			'due date': {'method': self._handle_date_maturity},
			'currency_id': {'method': self._handle_currency},
			'tax account': {'method': self._handle_tax_code},
			'tax_code_id': {'method': self._handle_tax_code},
			'analytic_account_id': {'method': self._handle_analytic_account},
			'analytics_id' : {'method': self._handle_analytics_account},
			'type_document_it' :{'method': self._handle_type_document},
			'means_payment_it':{'method': self._handle_means_payment},
			'amount_currency':{'method': self._handle_amount_currency},
			'seller_emit_id':{'method': self._handle_seller_emit_id},
			'date_emit': {'method': self._handle_date_emit},
		}
		return res
	...
	def _handle_orm_many2one(self, field, line, move, aml_vals,
		...
		if not aml_vals.get(orm_field):
			...
			if val is False:
				...
				aml_vals[orm_field] = val
	def _handle_seller_emit_id(self, field, line, move, aml_vals):
		if not aml_vals.get('user_id'):
			input = line[field]
			...
			dom = []
			dom_ref = dom + [('nro_documento', '=', input)]
			partners = partner_mod.search(dom_ref)
			if not partners:
				dom_name = dom + [('name', '=', input)]
				partner = partner_mod.search(dom_name)
				if not partner:
					msg = _("Users '%s' not found !") % input
					self._log_line_error(line, msg)
					return
			elif len(partners) > 1:
				msg = _("Multiple users with Reference "
						"or Name '%s' found !") % input
				self._log_line_error(line, msg)
				return
			else:
				users=user_mod.search([('partner_id','=',partners[0].id)])
				if not users:
					msg = _("Users '%s' not found !") % input
					self._log_line_error(line, msg)
					return
					...
					user = users[0]
					aml_vals['seller_emit_id'] = user.id
	...
	def _handle_amount_currency(self, field, line, move, aml_vals):
		if 'amount_currency' not in aml_vals:
			amount_currency = str2float(line[field], self.decimal_separator)
			aml_vals['amount_currency'] = amount_currency
	...
	def _handle_date_emit(self, field, line, move, aml_vals):
		if not aml_vals.get('date_emit'):
			due = line[field]
			try:
				datetime.strptime(due, '%Y-%m-%d')
				aml_vals['date_emit'] = due
			except:
				msg = _("Incorrect data format for field '%s' "
						"with value '%s', "
						" should be YYYY-MM-DD") % (field, due)
	...
	def _handle_analytics_account(self, field, line, move, aml_vals):
		if not aml_vals.get('analytics_account_id'):
			ana_mod = self.env['account.analytic.plan.instance']
			input = line[field]
			domain = []
			analytic_accounts = ana_mod.search(
				domain + [('code', '=', input)])
			if len(analytic_accounts) == 1:
				aml_vals['analytics_id'] = analytic_accounts.id
			else:
				analytic_accounts = ana_mod.search(
					domain + [('name', '=', input)])
				if len(analytic_accounts) == 1:
					aml_vals['analytics_id'] = analytic_accounts.id
			if not analytic_accounts:
				msg = _("Invalid Analytics Account '%s' !") % input
				self._log_line_error(line, msg)
			elif len(analytic_accounts) > 1:
				msg = _("Multiple Analytics Accounts found "
						"that match with '%s' !") % input
				self._log_line_error(line, msg)


	def _handle_type_document(self, field, line, move, aml_vals):
		if not aml_vals.get('type_document_it'):
			ana_mod = self.env['einvoice.catalog.01']
			input = line[field]
			domain = []
			analytic_accounts = ana_mod.search(
				domain + [('code', '=', input)])
			if len(analytic_accounts) == 1:
				aml_vals['type_document_it'] = analytic_accounts.id
			if not analytic_accounts:
				msg = _("Invalid Type Document '%s' !") % input
				self._log_line_error(line, msg)
			elif len(analytic_accounts) > 1:
				msg = _("Multiple Type Document found "
						"that match with '%s' !") % input
				self._log_line_error(line, msg)

	def _handle_means_payment(self, field, line, move, aml_vals):
		if not aml_vals.get('means_payment_it'):
			ana_mod = self.env['einvoice.means.payment']
			input = line[field]
			domain = []
			analytic_accounts = ana_mod.search(
				domain + [('code', '=', input)])
			if len(analytic_accounts) == 1:
				aml_vals['means_payment_it'] = analytic_accounts.id
			if not analytic_accounts:
				msg = _("Invalid Means Payment '%s' !") % input
				self._log_line_error(line, msg)
			elif len(analytic_accounts) > 1:
				msg = _("Multiple Means Payment found "
						"that match with '%s' !") % input
				self._log_line_error(line, msg)

	def _handle_fefective(self, field, line, move, aml_vals):
		if not aml_vals.get('fefectivo_id'):
			ana_mod = self.env['account.config.efective']
			input = line[field]
			domain = []
			analytic_accounts = ana_mod.search(
				domain + [('code', '=', input)])
			if len(analytic_accounts) == 1:
				aml_vals['fefectivo_id'] = analytic_accounts.id
			if not analytic_accounts:
				msg = _("Invalid Flujo Efective '%s' !") % input
				self._log_line_error(line, msg)
			elif len(analytic_accounts) > 1:
				msg = _("Multiple Flujo Efective found "
						"that match with '%s' !") % input
				self._log_line_error(line, msg)
	...
	def aml_import(self):
		...
		accounts = self.env['account.account'].search([
			('user_type_id.type', 'not in', ['view', 'consolidation', 'closed']),
			('company_id', '=', move.company_id.id)
			])
